...
def Workflow_1(config):
    if config.fabric == "emr":
        ...
        step_adder = EmrAddStepsOperator(task_id = "Workflow_1AddSteps", job_flow_id = job_flow_id, aws_conn_id = "aws_default_pankaj", steps = spark_steps, trigger_rule = "all_success")
        step_checker = EmrStepSensor(task_id = "Workflow_1WatchSteps", job_flow_id = job_flow_id, step_id = "{{ task_instance.xcom_pull(task_ids='Workflow_1AddSteps', key='return_value')[0] }}", aws_conn_id = "aws_default_pankaj")
